...
const createPhongValidation = [
  ...
  body("gioi_tinh").optional().isIn(["male", "female"]).withMessage("Giới tính phải là male hoặc female"),
  body("trang_thai")
    .optional()
...
const updatePhongValidation = [
  ...
  body("ten_phong")
    ...
    .isLength({ min: 1, max: 50 })
    .withMessage("Tên phòng phải có độ dài từ 1-50 ký tự"),
  body("tang").optional().isInt({ min: 1, max: 50 }).withMessage("Tầng phải là số nguyên từ 1-50"),
  body("sl_max").optional().isInt({ min: 1, max: 20 }).withMessage("Số lượng tối đa phải là số nguyên từ 1-20"),
  body("gioi_tinh").optional().isIn(["male", "female"]).withMessage("Giới tính phải là male hoặc female"),
  body("trang_thai")
    .optional()
    .isIn(["available", "partially_occupied", "full", "maintenance", "unavailable"])
...
const queryValidation = [
  ...
  query("id_loai_phong").optional().isInt({ min: 1 }).withMessage("ID loại phòng phải là số nguyên dương"),
  query("tang").optional().isInt({ min: 1 }).withMessage("Tầng phải là số nguyên dương"),
 ...
 * @access  Private
 */
router.get("/", authMiddleware, queryValidation, validateRequest, phongController.getAllPhong);
 ...
 * @access  Private
 */
router.get("/:id", authMiddleware, idValidation, validateRequest, phongController.getPhongById);
 ...
 */
router.post("/", authMiddleware, createPhongValidation, validateRequest, phongController.createPhong);
 ...
 * @access  Private (Admin only)
 */
router.put("/:id", authMiddleware, updatePhongValidation, validateRequest, phongController.updatePhong);
 ...
 * @access  Private (Admin only)
 */
router.delete("/:id", authMiddleware, idValidation, validateRequest, phongController.deletePhong);
 ...
 * @access  Private
 */
router.get("/available", authMiddleware, availabilityValidation, validateRequest, phongController.getAvailablePhong);
 ...
 * @access  Private
 */
router.get("/:id/availability", authMiddleware, idValidation, validateRequest, phongController.checkPhongAvailability);
